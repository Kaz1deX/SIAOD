...
{
    if (first < last) // условие на то, чтобы начать сортировку массива, если оно не выполняется,
...
void qs(vector<int>& A, int first, int last, int& CF, int& MF)
{
...
int main()
{
    ...
    int MF = 0;
    cin >> n;
    vector<int> A = {1,2,3,4,0,5,6,7,8,9};
    //Random(A, n);
